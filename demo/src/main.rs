//! Demo: interactive prompts using the promptt library.
//!
...
fn main() -> io::Result<()> {
    ...
    // 1. text – single-line input, optional default
    // 2. confirm – Y/n or N/y
    // 3. number – integer or float, optional min/max
    // 4. select – pick one from choices (by index or title)
    // 5. toggle – on/off (customizable labels)
    // 6. list – comma-separated input, returns Vec<String>
